...
# ---------------------------------------------------------------------------
# Custom straight staircase terrain functions
# ---------------------------------------------------------------------------

def straight_ascending_stairs_terrain(
    difficulty: float, cfg: "StraightAscendingStairsTerrainCfg"
) -> tuple[list[trimesh.Trimesh], np.ndarray]:
    """Generate a straight ascending staircase in the +x direction.

    Layout (side view, x-z plane):
        flat spawn area | stairs going UP | flat top area
        z=0             |  z=0..h_total  | z=h_total

    Robot spawns at (flat_start/2, H/2, 0) facing +x, and walks up the stairs.

    Args:
        difficulty: Terrain difficulty in [0, 1]. Interpolates step_height_range.
        cfg: Terrain configuration.

    Returns:
        Tuple of (mesh_list, origin). Origin is the robot spawn point.
    """
    W, H = cfg.size
    ...
    num_steps = max(1, int(stair_section / step_width))
    total_height = num_steps * step_height

    y_center = H / 2.0
    thickness = 0.2  # box thickness below ground for solid base

    meshes = []

    # Flat spawn area at z=0 (box top surface = z=0)
    meshes.append(trimesh.creation.box(
        (flat_start, stair_width, thickness),
        trimesh.transformations.translation_matrix([flat_start / 2, y_center, -thickness / 2]),
    ))

    # Staircase: each step k occupies x=[flat_start+k*sw, flat_start+(k+1)*sw]
    # Top surface at z=(k+1)*step_height, solid from z=-thickness to that height.
    for k in range(num_steps):
        box_height = (k + 1) * step_height + thickness
        box_x = flat_start + (k + 0.5) * step_width
        box_z = ((k + 1) * step_height - thickness) / 2.0
        meshes.append(trimesh.creation.box(
            (step_width, stair_width, box_height),
            trimesh.transformations.translation_matrix([box_x, y_center, box_z]),
        ))

    # Flat top area at z=total_height
    top_x = flat_start + num_steps * step_width
    meshes.append(trimesh.creation.box(
        (flat_end, stair_width, thickness),
        trimesh.transformations.translation_matrix([top_x + flat_end / 2, y_center, total_height - thickness / 2]),
    ))

    # Robot spawn origin: center of flat spawn area, at ground level
    origin = np.array([flat_start / 2.0, y_center, 0.0])
...
def straight_descending_stairs_terrain(
    ...
    """Generate a straight descending staircase in the +x direction.

    Layout (side view, x-z plane):
        flat spawn area | stairs going DOWN | flat bottom area
        z=h_total       |  z=h_total..0    | z=0

    Robot spawns at (flat_start/2, H/2, total_height) facing +x, and walks down.
    ...
    # Flat top area (spawn area at z=total_height)
    meshes.append(trimesh.creation.box(
        (flat_start, stair_width, thickness),
        trimesh.transformations.translation_matrix([flat_start / 2, y_center, total_height - thickness / 2]),
    ))

    # Staircase: step k (0-indexed from top) at x=[flat_start+k*sw, flat_start+(k+1)*sw]
    # Top surface descends: z = total_height - (k+1)*step_height
    # Solid from z=-thickness to z=total_height-(k+1)*step_height
    for k in range(num_steps):
        top_z = total_height - (k + 1) * step_height
        box_height = top_z + thickness
        if box_height <= 0:
            continue
        box_x = flat_start + (k + 0.5) * step_width
        box_z = (top_z - thickness) / 2.0
        meshes.append(trimesh.creation.box(
            (step_width, stair_width, box_height),
            trimesh.transformations.translation_matrix([box_x, y_center, box_z]),
        ))

    # Flat bottom area at z=0
    bottom_x = flat_start + num_steps * step_width
    meshes.append(trimesh.creation.box(
        (flat_end, stair_width, thickness),
        trimesh.transformations.translation_matrix([bottom_x + flat_end / 2, y_center, -thickness / 2]),
    ))

    # Robot spawn origin: center of flat top area
    origin = np.array([flat_start / 2.0, y_center, total_height])
    return meshes, origin


# ---------------------------------------------------------------------------
# Custom terrain config classes
# ---------------------------------------------------------------------------

@configclass
class StraightAscendingStairsTerrainCfg(SubTerrainBaseCfg):
    """Configuration for a straight ascending staircase terrain."""

    function = straight_ascending_stairs_terrain

    step_height_range: tuple[float, float] = MISSING
    """Min and max step height (m). Use (h, h) for fixed height."""

    step_width: float = 0.28
    """Horizontal depth of each step (m)."""

    stair_width: float = 6.0
    """Width of the stair corridor in the y-direction (m)."""

    flat_start_length: float = 1.5
    """Flat spawn area length in the x-direction (m)."""

    flat_end_length: float = 1.5
    """Flat landing area length at the top (m)."""


@configclass
class StraightDescendingStairsTerrainCfg(SubTerrainBaseCfg):
    """Configuration for a straight descending staircase terrain."""
