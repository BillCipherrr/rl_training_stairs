...
@configclass
class DeeproboticsLite3StairsPIEPPORunnerCfg(RslRlOnPolicyRunnerCfg):
    """PPO configuration for Lite3 stairs climbing with PIE Estimator.

    Uses the PIEOnPolicyRunner which integrates the PIE dual-level
    implicit-explicit estimator into the training loop.

    Key differences from standard PPO:
    - class_name points to PIEOnPolicyRunner
    - Includes pie_cfg with estimator hyperparameters
    - Policy network input: 45 (proprio) + 103 (PIE injection) = 148 dims
    - Critic network input: standard critic with height scan
    - Separate PIE loss (reconstruction + estimation + KL) optimized alongside PPO

    Architecture:
        Actor:  MLP [148 → 512 → 256 → 128 → 12]  (PIE-augmented input)
        Critic: MLP [390 → 512 → 256 → 128 → 1]   (privileged with height scan)
        PIE:    ProprioEncoder + DepthEncoder + FusionTransformer + GRU + Heads (~1.26M params)
    """
    num_steps_per_env = 32
    max_iterations = 20000
    save_interval = 100
    experiment_name = "deeprobotics_lite3_stairs_pie"
    empirical_normalization = False
    clip_actions = 100
    # Actor handles 148-dim input (45 proprio + 103 PIE injection)
    # Critic handles standard privileged input with height scan
    policy = RslRlPpoActorCriticCfg(
        init_noise_std=1.0,
        noise_std_type="log",
        actor_hidden_dims=[512, 256, 128],
